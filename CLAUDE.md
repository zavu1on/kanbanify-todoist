...
- Kanban-статус задачи (без статуса / todo / in-progress / completed) хранится не как отдельная сущность, а как **зарезервированный Todoist-тег** на задаче — этим kanban-фреймворк отличается от нативного статуса `completed` в Todoist (задача с тегом `completed` != выполненная задача в Todoist). Если зарезервированных тегов на задаче несколько, актуальным считается самый правый по порядку колонок: `completed`, затем `in-progress`, затем `todo`.
