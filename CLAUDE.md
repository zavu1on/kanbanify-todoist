...
Исключение — `git-commit`, `accept-adr` и `todoist-sdk`: это самописные скиллы (первые два скопированы из другого проекта пользователя, `todoist-sdk` — написан для этого репозитория на основе стороннего источника), а не установленные из реестра skills.sh. Поэтому они **не** фигурируют в `skills-lock.json` — это осознанно, лок-файл отражает только внешние источники.
