...
export class ArchiveProjectUseCase implements UseCase<string, void> {
  ...
  async execute(projectId: string): Promise<void> {
    ...
    // Validates the Inbox-protection invariant before the API call —
    // archiving isn't a field update, so it's persisted through the
    // gateway's specific `archive` method, not `save`.
    project.archive();

    await this.projectGateway.archive(accessToken.value, projectId);
  }
}
