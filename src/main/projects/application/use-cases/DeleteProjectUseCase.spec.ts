...
describe("DeleteProjectUseCase", () => {
  ...
  it("loads the project and deletes it through the gateway", async () => {
    ...
    expect(projectGateway.delete).toHaveBeenCalledWith(token.value, "1");
  });

  it("throws InboxProjectProtectedError and never calls the gateway's delete for the Inbox project", async () => {
    ...
    expect(projectGateway.delete).not.toHaveBeenCalled();
  });
});
